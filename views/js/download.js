...
const makeRequest = () => {
    const url = readUrl()
    const enpoint = "https://xuantruongdev.id.vn/tiktok/video?url="
    if (url) {
...
const renderData = (data) => {
    ...
    const videoSource = data['video']['no_watermark']
    const videoPoster = data['video']['thumbnail_src']
    const authorName = data['channel']['nickname']
    const authorUsername = data['channel']['username']
    // const authorDetails = data['author']['uid']
    const authorAvatar = data['channel']['avatar_thumb']['url_list'][0]
    const authorVideos = "https://www.tiktok.com/@" + data['channel']['username']
    const videoDownload = data['video']['no_watermark']
    const videoDownloadWM = data['video']['watermark']
    const musicDownload = data['music']['mp3']['uri']
    const musicCover = data["music"]['cover_hd'][0]
    const musicTitle = data["music"]['title']
    const musicAuthor = data["music"]['author']
...
const getMediaBlobs = (data) => {
    ...
    const video = data['video']['no_watermark']
    const video_wm = data['video']['watermark']
    const music = data["music"]['mp3']['uri']
    getBlob("video", video)
    getBlob("music", music)
...
const waitForBlobToDownload = (blobKey, button) => {
    ...
    if (!BLOBS.hasOwnProperty(blobKey)) {
        ...
        let filename = DATA['channel']['uid'] + "___" + DATA['video']['title'] + "___"
        if (blobKey == "video") {
            filename += "NO_WM"
            var extension = ".mp4"
            ...
            filename = DATA["music"]['author'] + "___" + DATA["music"]['title']
            var extension = ".mp3"
        }
...
// const videoSource = data['video']['no_watermark']
    ...
    const authorName = data['channelchannelchannel']['nickname']
    const authorUsername = data['channel']['uid']
    // const authorDetails = data['author']['uid']
    const authorAvatar = data['channel']['avatar_thumb']['url_list'][0]
    const authorVideos = "https://www.tiktok.com/@" + data['channel']['uid']
    const videoDownload = data['video']['no_watermark']
    const videoDownloadWM = data['video']['watermark']
    const musicDownload = data['music']['mp3']['uri']
    const musicCover = data["music"]['mp3']['uri']
    const musicTitle = data["music"]['title']
    const musicAuthor = data["music"]['author']
